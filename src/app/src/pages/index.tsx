...
export default function App() {
  ...
  function handleChangeEndpointGroup(group: string, endpoint: Endpoint) {
    ...
    const targetEndpoint = endpoints.find((e) => e.path === endpoint.path && e.method === endpoint.method);
  ...
  async function handleSync() {
    setBusy(true);
    try {
      const syncPromise = groupEndpoints({
        entrypoint,
        targetDir,
      ...
      <div className="mb-2 flex justify-between items-center gap-2">
        ...
        {(entrypoint || targetDir) && (
          <DataList.Root>
            {entrypoint && (
              <DataList.Item>
                <DataList.Label className="text-white">Entrypoint:</DataList.Label>
                <DataList.Value className="text-white font-bold">{entrypoint}</DataList.Value>
              </DataList.Item>
            )}
            {targetDir && (
              <DataList.Item>
                <DataList.Label className="text-white">Target directory:</DataList.Label>
                <DataList.Value className="text-white font-bold">{targetDir}</DataList.Value>
              </DataList.Item>
            )}
          </DataList.Root>
        )}
      </div>
      <div className="bg-dark-3 text-dark" style={{ height: 'calc(100vh - 170px)', width: '100hh' }}>
        <ApiTree
