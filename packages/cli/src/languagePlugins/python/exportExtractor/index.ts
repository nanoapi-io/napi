...
export class PythonExportExtractor {
  ...
  private cache = new Map<
    string,
    { symbols: Symbol[]; publicSymbols: string[] }
  >(); // Caching extracted symbols for efficiency
   ...
   */
  private getPublicSymbols(fileNode: Parser.SyntaxNode): string[] {
  ...
  public getSymbols(filePath: string) {
    ...
    symbols.push(...this.getClass(fileNode));
    symbols.push(...this.getFunctions(fileNode));
    symbols.push(...this.getVariable(fileNode));

    const publicSymbols = this.getPublicSymbols(fileNode);
